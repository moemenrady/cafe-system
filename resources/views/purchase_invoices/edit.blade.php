...
@section('title', 'تعديل فاتورة شراء رقم: ' . $purchaseInvoice->invoice_number)
...
<div class="space-y-6 max-w-6xl mx-auto pb-16" dir="rtl">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-gray-100 shadow-xs">
        <div class="flex items-center gap-3.5">
            <a href="{{ route('purchase-invoices.show', $purchaseInvoice->id) }}"
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">تعديل بيانات الفاتورة والأصناف المسجلة وإعادة ضبط رصيد المخزن تلقائياً</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <button type="submit" form="purchaseInvoiceEditForm" id="saveInvoiceBtn"
                ...
                <span>حفظ التعديلات وتحديث المخزن</span>
            </button>
        </div>
    </div>
    ...
    {{-- تنبيه خاص بالتعديل وتأثيره على رصيد المخزون --}}
    <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 flex items-start gap-3">
        <i class="fa-solid fa-triangle-exclamation text-amber-600 text-base mt-0.5 shrink-0"></i>
        <div class="space-y-1">
            <span class="font-black block">تنبيه متعلق بأرصدة المخزون:</span>
            <p class="text-amber-800 leading-relaxed text-[11px]">
                تعديل الفاتورة سيقوم تلقائياً بخصم الكميات القديمة المسجلة بهذه الفاتورة من المخزن ثم إضافة الكميات الجديدة بعد التعديل لضمان دقة الرصيد المخزني دائماً.
            </p>
        </div>
    </div>

    {{-- ==================== نموذج تعديل فاتورة الشراء ==================== --}}
    <form action="{{ route('purchase-invoices.update', $purchaseInvoice->id) }}" method="POST" id="purchaseInvoiceEditForm" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 mb-1.5">
                        رقم الفاتورة (لا يمكن تغييره)
                    </label>
                    <input type="text" value="{{ $purchaseInvoice->invoice_number }}" readonly
                        class="w-full bg-gray-100 border border-gray-200 rounded-xl px-3 py-2.5 text-xs text-gray-600 font-mono font-bold focus:outline-hidden cursor-not-allowed">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="supplier_name" id="supplierNameInput"
                        value="{{ old('supplier_name', $purchaseInvoice->supplier_name) }}" required
                ...
                <div>
                    ...
                    <select name="payment_method" id="paymentMethodSelect"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs text-gray-900 font-bold focus:ring-2 focus:ring-blue-500 focus:outline-hidden">
                        <option value="cash" {{ old('payment_method', $purchaseInvoice->payment_method) === 'cash' ? 'selected' : '' }}>💵 كاش نقدي</option>
                        <option value="instapay" {{ old('payment_method', $purchaseInvoice->payment_method) === 'instapay' ? 'selected' : '' }}>📱 إنستا باي</option>
                        <option value="card" {{ old('payment_method', $purchaseInvoice->payment_method) === 'card' ? 'selected' : '' }}>💳 فيزا / شبكة</option>
                        <option value="credit" {{ old('payment_method', $purchaseInvoice->payment_method) === 'credit' ? 'selected' : '' }}>⏳ آجل (حساب مورد)</option>
                        <option value="bank_transfer" {{ old('payment_method', $purchaseInvoice->payment_method) === 'bank_transfer' ? 'selected' : '' }}>🏦 تحويل بنكي</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                ...
                <div>
                    ...
                    <select name="payment_status" id="paymentStatusSelect" onchange="handlePaymentStatusChange()"
                        class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs text-gray-900 font-bold focus:ring-2 focus:ring-blue-500 focus:outline-hidden">
                        <option value="paid" {{ old('payment_status', $purchaseInvoice->payment_status) === 'paid' ? 'selected' : '' }}>✅ مدفوعة بالكامل</option>
                        <option value="partial" {{ old('payment_status', $purchaseInvoice->payment_status) === 'partial' ? 'selected' : '' }}>⚠️ مدفوعة جزئياً</option>
                        <option value="unpaid" {{ old('payment_status', $purchaseInvoice->payment_status) === 'unpaid' ? 'selected' : '' }}>❌ غير مدفوعة (آجلة بالكامل)</option>
                    </select>
                </div>
                ...
                <div id="paidAmountContainer">
                    ...
                    <input type="number" step="0.01" min="0" name="paid_amount" id="paidAmountInput"
                        value="{{ old('paid_amount', $purchaseInvoice->paid_amount) }}" oninput="calculateTotals()"
                ...
                <div>
                    ...
                    <input type="text" id="remainingAmountDisplay" readonly value="{{ number_format($purchaseInvoice->remaining_amount, 2) }} ج.م"
        ...
        {{-- 2. قسم تفاصيل وأصناف الفاتورة المتعددة --}}
        <div class="bg-white rounded-3xl p-5 sm:p-6 border border-gray-100 shadow-xs space-y-5">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-gray-100 pb-3">
                <div>
                    ...
                    <p class="text-[11px] text-gray-400 mt-0.5">يمكنك إضافة أو تعديل أو حذف أصناف من هذه الفاتورة</p>
                </div>
            ...
            <div id="itemsContainer" class="space-y-4">
                {{-- سيتم تعبئتها تلقائياً بواسطة الجافاسكريبت بالبيانات الحالية --}}
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-white rounded-3xl p-5 border border-gray-100 shadow-xs space-y-2">
                ...
                <textarea name="notes" rows="4" placeholder="أدخل أي ملاحظات خاصة بالتسليم أو شروط المورد أو تفاصيل الشحنة..."
                    class="w-full bg-gray-50 border border-gray-200 rounded-2xl p-3 text-xs text-gray-800 placeholder-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500">{{ old('notes', $purchaseInvoice->notes) }}</textarea>
            </div>
            ...
            <div class="bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-3xl p-5 shadow-xl space-y-4">
                ...
                <div class="space-y-2.5 text-xs">
                    <div class="flex justify-between items-center text-gray-300">
                        ...
                        <span class="font-mono font-bold text-white text-sm" id="subtotalDisplay">{{ number_format($purchaseInvoice->total_amount, 2) }} ج.م</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center text-gray-300 gap-2">
                        ...
                        <input type="number" step="0.01" min="0" name="discount" id="discountInput"
                            value="{{ old('discount', $purchaseInvoice->discount) }}" oninput="calculateTotals()"
                    ...
                    <div class="flex justify-between items-center text-gray-300 gap-2">
                        ...
                        <input type="number" step="0.01" min="0" name="tax" id="taxInput"
                            value="{{ old('tax', $purchaseInvoice->tax) }}" oninput="calculateTotals()"
                    ...
                    <div class="border-t border-gray-700/80 pt-2.5 flex justify-between items-baseline">
                        ...
                        <span class="text-2xl font-black font-mono text-emerald-400" id="netTotalDisplay">{{ number_format($purchaseInvoice->net_amount, 2) }} <span class="text-xs">ج.م</span></span>
                    </div>

                    <div class="border-t border-gray-700/60 pt-2 flex justify-between items-center text-[11px] text-gray-400">
                        <span>المبلغ المدفوع:</span>
                        <span class="font-mono font-bold text-gray-200" id="paidDisplay">{{ number_format($purchaseInvoice->paid_amount, 2) }} ج.م</span>
                    </div>

                    <div class="flex justify-between items-center text-[11px] text-rose-400 font-bold">
                        <span>المتبقي للمورد:</span>
                        <span class="font-mono" id="remainingDisplay">{{ number_format($purchaseInvoice->remaining_amount, 2) }} ج.م</span>
                    </div>
                </div>
                ...
                <button type="submit" id="saveInvoiceBottomBtn"
                    ...
                    <span>تحديث الفاتورة وتعديل المخزون</span>
                </button>
            </div>
        </div>
...
<script>
    ...
    // الأصناف الحالية المحفوظة بالفاتورة
    const initialItemsData = @json($initialItems);

    let rowCounter = 0;

    // إضافة صف صنف جديد أو تعبئته بالبيانات الموجودة مسبقاً
    function addNewItemRow(initialData = null) {
        ...
        const initialName = initialData ? (initialData.name || '') : '';
        const initialInvId = initialData ? (initialData.inventory_item_id || '') : '';
        const initialUnit = initialData ? (initialData.unit || 'قطعة') : 'قطعة';
        const initialCat = initialData ? (initialData.category || 'عام') : 'عام';
        const initialQty = initialData ? (initialData.quantity || 1) : 1;
        const initialPrice = initialData ? (parseFloat(initialData.unit_price || 0).toFixed(2)) : '0.00';
        const initialNotes = initialData ? (initialData.notes || '') : '';
        ...
        rowCard.innerHTML = `
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-start">
                ...
                <div class="lg:col-span-4 relative">
                    ...
                    <input type="hidden" name="items[${rowIndex}][inventory_item_id]" id="inv_id_${rowIndex}" value="${initialInvId}">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" list="unitsList" name="items[${rowIndex}][unit]" id="unit_${rowIndex}" required
                        value="${escapeHtml(initialUnit)}" placeholder="كجم، لتر، علبة..."
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" list="catsList" name="items[${rowIndex}][category]" id="category_${rowIndex}"
                        value="${escapeHtml(initialCat)}" placeholder="القسم..."
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="number" step="0.01" min="0.01" name="items[${rowIndex}][quantity]" id="qty_${rowIndex}" required
                        value="${initialQty}" oninput="calculateRowTotal(${rowIndex})"
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="number" step="0.01" min="0" name="items[${rowIndex}][unit_price]" id="price_${rowIndex}" required
                        value="${initialPrice}" oninput="calculateRowTotal(${rowIndex})"
                        class="w-full bg-white border border-gray-200 rounded-xl px-2.5 py-2 text-xs text-gray-900 font-mono font-black text-center focus:ring-2 focus:ring-blue-500 focus:outline-hidden">
                </div>
            </div>
            ...
            <div class="mt-3 pt-2.5 border-t border-gray-200/60 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div class="flex-1 w-full sm:w-auto">
                    <input type="text" name="items[${rowIndex}][notes]" value="${escapeHtml(initialNotes)}" placeholder="ملاحظات الصنف (اختياري)..."
        ...
        if (initialInvId) {
            const matched = dbInventoryItems.find(i => i.id == initialInvId);
            if (matched) {
                const stockInfo = document.getElementById(`stock_info_${rowIndex}`);
                stockInfo.classList.remove('hidden');
                stockInfo.textContent = `(الرصيد بالمخزن: ${matched.quantity} ${matched.unit})`;
            }
        }
    ...
    function handleItemSearch(rowIndex) {
        ...
        if (exactMatch) {
            ...
            newBadge.classList.add('hidden');
            stockInfo.classList.remove('hidden');
            stockInfo.textContent = `(الرصيد بالمخزن: ${exactMatch.quantity} ${exactMatch.unit})`;
        } else {
            invIdInput.value = '';
    ...
    function selectExistingItem(rowIndex, itemId) {
        ...
        const stockInfo = document.getElementById(`stock_info_${rowIndex}`);
        stockInfo.classList.remove('hidden');
        stockInfo.textContent = `(الرصيد بالمخزن: ${item.quantity} ${item.unit})`;
    ...
    // تهيئة الصفحة بملء الأصناف الحالية
    document.addEventListener('DOMContentLoaded', function() {
        if (initialItemsData && initialItemsData.length > 0) {
            initialItemsData.forEach(item => addNewItemRow(item));
        } else {
            addNewItemRow();
        }
