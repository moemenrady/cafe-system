...
<div class="container mx-auto px-6 py-8" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <a href="{{ route('inventory.create') }}" class="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-blue-200 hover:bg-blue-500 hover:shadow-none transition-all duration-300">
            إضافة خامة جديدة +
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div class="space-y-1">
                ...
                <h3 class="text-2xl font-bold text-yellow-600">{{ $items->where('quantity', '<=', 'reorder_level')->count() }} أصناف</h3>
            </div>
    ...
    <!-- التنبيهات والرسائل -->
    @if(session('success'))
        <div class="bg-green-50 border-r-4 border-green-500 text-green-700 p-4 rounded-xl mb-6 shadow-sm flex items-center">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-max w-full table-auto text-right">
                <thead>
                    <tr class="bg-gray-50 text-gray-500 uppercase text-xs font-bold tracking-wider border-b border-gray-100">
                ...
                <tbody class="text-gray-600 text-sm font-light divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/80 transition-colors duration-200">
                            ...
                            <td class="py-4 px-6 text-center">
                                @if($item->quantity <= $item->reorder_level)
                                    <span class="bg-red-50 text-red-600 py-1 px-3 rounded-full text-xs font-bold border border-red-100">⚠️ محتاج شراء فوراً</span>
                                @else
                                    <span class="bg-green-50 text-green-600 py-1 px-3 rounded-full text-xs font-medium border border-green-100">✅ آمن</span>
                                @endif
                            </td>
                            ...
                            <td class="py-4 px-6 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="openAdjustModal({{ $item->id }}, '{{ $item->name }}', '{{ $item->unit }}', 'restock')" 
                                    ...
                                    <button onclick="openAdjustModal({{ $item->id }}, '{{ $item->name }}', '{{ $item->unit }}', 'waste')" 
                            ...
                            <td class="py-4 px-6 text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('inventory.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800 font-bold bg-blue-50 hover:bg-blue-100 px-2.5 py-1 rounded-lg transition-all">
                                        تعديل
                                    </a>
                                    <form action="{{ route('inventory.destroy', $item->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه المادة بالكامل؟ سيتم فحص ارتباطها بالمنيو أولاً.');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800 font-bold bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded-lg transition-all">
                                            حذف
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-8 px-6 text-center text-gray-400 font-medium text-base">المخزن فارغ حالياً. قم بإضافة خامات لتبدأ العمل.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="adjustModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form id="adjustForm" method="POST" action="">
                ...
                <div class="bg-white px-6 pt-6 pb-4 sm:p-6 sm:pb-4">
                    <div class="flex items-start gap-4">
                        <div class="mt-3 text-right sm:mt-0 w-full">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500" id="modalDescription">قم بإدخال الكمية المطلوبة بدقة ليتم إدراجها في دفتر الحركات.</p>
                            </div>

                            <div class="mt-5">
                                <label for="amount" class="block text-sm font-semibold text-gray-700 mb-2">الكمية (<span id="modalUnit" class="text-blue-600 font-bold"></span>)</label>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button type="submit" id="submitBtn" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2.5 text-base font-medium text-white sm:ml-3 sm:w-auto transition-colors duration-200">
                        تأكيد وحفظ
                    </button>
                    <button type="button" onclick="closeAdjustModal()" class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors duration-200">
                        إلغاء
                    </button>
                </div>
            </form>
...
<script>
    function openAdjustModal(itemId, itemName, itemUnit, actionType) {
        ...
        if (actionType === 'restock') {
            ...
            submitBtn.className = "w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2.5 text-base font-medium text-white bg-emerald-600 hover:bg-emerald-500 sm:w-auto transition-colors duration-200";
        } else {
            title.innerText = `تسجيل هالك وتالف لـ [${itemName}]`;
            desc.innerText = "أدخل كمية المواد التي فسدت، انتهت صلاحيتها أو سُكبت ليتم خصمها تلقائياً من الـ Inventory.";
            submitBtn.className = "w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2.5 text-base font-medium text-white bg-amber-600 hover:bg-amber-500 sm:w-auto transition-colors duration-200";
        }
